...
use windows::Win32::Graphics::Gdi::{
    CreateCompatibleDC, DeleteDC, DeleteObject, GetDIBits, GetObjectW, HBITMAP, HDC,
    SelectObject, BI_RGB, DIB_RGB_COLORS, BITMAP, BITMAPINFO, BITMAPINFOHEADER,
};
...
const NAME_BLACKLIST: &[&str] = &[
    // 卸载/安装
    "uninstall", "unins", "卸载", "setup", "安装", "installer",
    // 文档/帮助
    "readme", "help", "helper", "manual", "手册", "documentation", "docs",
    "changelog", "release notes", "更新日志", "版本说明",
    // 配置/工具
    "config", "configure", "配置", "settings", "选项",
    // 开发辅助
    "command prompt", "developer command", "developer powershell",
    "native tools command", "cross tools command",
    "命令提示符",
    // 系统管理（非日常使用）
    "diagnostic", "performance", "powershell",
    "regedit", "msconfig", "task manager", "resource monitor",
    "event viewer", "computer management", "disk management", "device manager",
    "services", "windows update", "windows security", "windows defender",
    "windows firewall",
    "任务管理器", "资源监视器", "事件查看器", "计算机管理", "磁盘管理", "设备管理器",
    "windows 更新", "windows 安全中心", "windows 防火墙", "运行",
    // Windows 系统/SDK 工具
    "windows", "microsoft",
    "application verifier", "debugging", "global flags",
    "gpuview", "log parser", "wfetch", "tinyget", "wcat",
    "iisstate", "iiscertdeploy", "selfssl", "permissions verifier",
    "metabase explorer", "security configuration",
    ...
    if let Some(target) = resolve_lnk_target(&lp) {
        ...
        match read_pe_subsystem(target_path) {
            ...
            PeKind::NotPe => {
                ...
                if target_lower.ends_with(".msc") { return false; }
...
fn resolve_lnk_target(lnk_path: &str) -> Option<String> {
    ...
    let target = shortcut.relative_path()
        .as_ref()
        .and_then(|p| if p.is_empty() { None } else { Some(p.clone()) })
        .or_else(|| {
            shortcut.working_dir().as_ref().and_then(|wd| {
                shortcut.relative_path().as_ref().map(|rp| PathBuf::from(wd).join(rp).to_string_lossy().to_string())
            })
        })
        .or_else(|| {
            shortcut.icon_location().as_ref().and_then(|loc| if loc.is_empty() { None } else { Some(loc.clone()) })
        })?;
...
pub fn scan_and_save(conn: &Connection, _app_handle: &tauri::AppHandle) -> Result<ScanResult, String> {
    ...
    for app in seen.values() {
        if !is_real_app(&app.name, &app.path) { continue; }
    ...
    Ok(ScanResult { apps: filtered, new_count: new })
}

/// 清理数据库中不再符合过滤条件的旧条目
fn cleanup_stale_entries(conn: &Connection, current_apps: &[ScannedApp]) {
    // 收集当前扫描通过的所有路径
    let valid_paths: std::collections::HashSet<String> = current_apps
        .iter()
    ...
    let rows: Vec<(i64, String, String)> = match stmt.query_map([], |row| {
...
pub fn extract_and_cache_icon(app_path: &str, ah: &tauri::AppHandle) -> Option<String> {
    ...
    if cache_path.exists() && std::fs::metadata(&cache_path).map(|m| m.len() > 0).unwrap_or(false) {
...
fn extract_icon_to_png(exe_path: &str, output_path: &Path) -> Result<(), String> {
    ...
    // Step 1: Get icon handle using SHGetFileInfoW with SHGFI_ICON | SHGFI_LARGEICON
    ...
    png_writer.write_image_data(&pixel_data).map_err(|e| e.to_string())?;
...
fn get_bitmap_rgba(hbm: HBITMAP) -> Result<(u32, u32, Vec<u8>), String> {
    unsafe {
        ...
        let result = GetObjectW(hbm, std::mem::size_of::<BITMAP>() as i32, Some(&mut bm as *mut _ as *mut _));
        if result == 0 {
            let _ = DeleteDC(hdc);
        ...
        for y in 0..height as usize {
            ...
            flipped[dst_offset..dst_offset + stride].copy_from_slice(&pixels[src_offset..src_offset + stride]);
        }
