...
export default function AIChat({ onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/20" onClick={onClose}>
      <div className="w-[520px] h-[580px] flex flex-col rounded-2xl bg-popover border border-border shadow-2xl overflow-hidden animate-scale-in" onClick={e => e.stopPropagation()}>
        {/* 标题 */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-border/60">
          <div className="flex items-center gap-2">
            <Bot className="w-5 h-5 text-primary" />
            <span className="text-sm font-semibold">QuickStart AI</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-2 ${msg.role === "user" ? "justify-end" : ""}`}>
              ...
              <div className={`max-w-[80%] px-3 py-2 text-sm ${msg.role === "user"
          ...
          {streamingText && (
            <div className="flex gap-2">
              ...
              <div className="max-w-[80%] px-3 py-2 text-sm bg-muted text-foreground rounded-2xl rounded-bl-md shadow-sm">
        ...
        <div className="px-4 py-3 border-t border-border">
          <div className="flex gap-2">
            <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)}
              ...
              disabled={loading}
              className="flex-1 h-10 px-3 rounded-xl bg-secondary/60 border border-border/60 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/50 text-sm text-foreground placeholder:text-muted-foreground disabled:opacity-50" />
            ...
            <button onClick={sendMessage} disabled={loading || !input.trim()} className="p-2 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground shadow-sm transition-opacity disabled:opacity-50">
